// PromesBlueGuidePrefabBuilder.cs
// -----------------------------------------------------------------------------
// 사용법
...
// 3) 상단 메뉴 Tools > Promes > Create Blue Guide Prefab 클릭
// 4) Assets/Prefabs/UI/Promes_Blue_Guide.prefab 로 프리팹이 저장되고,
//    선택했던 Content 밑에 인스턴스가 바로 생성됩니다.
//
// * Brown/Green/Yellow 프리팹과 구조는 동일하되, 색상 스와치 섹션은 데이터가
...
public static class PromesBlueGuidePrefabBuilder
{
    ...
    private static readonly Color SectionHeadColor = new Color(0.12f, 0.45f, 0.85f, 1f); // 텍스트 임베딩 (파랑)
    private static readonly Color RuleHeadColor = new Color(0.12f, 0.45f, 0.85f, 1f);    // 프롬프트 결합 규칙 (파랑)
    ...
    private static readonly Color TableBorderColor = new Color(0.6f, 0.75f, 0.92f, 1f);
    private static readonly Color TableHeaderBgColor = new Color(0.12f, 0.45f, 0.85f, 1f); // 진한 파랑
    ...
    {
        new TableRow("소재/질감", "0.96", "물/바다, 얼음/결정, 홀로그램, 데님(청바지), 금속/크롬, 청경채/유리, 레이저, 디지털 화면"),
        new TableRow("조명/감정", "0.94", "차가운 광원, 고독, 신뢰, 냉정, 신비로움, 서늘함, 평온, 우울(Blue)"),
        new TableRow("문체/시대", "0.91", "SF/미래도시, 사이버펑크, 첨단 IT/AI, 심해 탐사, 메카닉, 미스터리 느와르"),
        new TableRow("분위기", "0.89", "정갈함, 미래지향적, 신비함, 이성적, 광활함, 정적, 정밀함"),
    };
    ...
    {
        new Rule("프롬프트에 (\"파란색\" + \"조명\" 또는 \"광원\" 또는 \"네온\")이 포함된 경우:", new string[]
        {
            "→ 주요 잠재 목표: 홀로그램 발광 / 사이버펑크 블루 림 라이트 / 차가운 광원 반응",
            "→ 색상 스펙트럼 범위: #00E5FF ~ #2979FF",
            "→ 표면 특성: 주위 오브젝트 표면으로의 차가운 빛 반사 및 은은한 블루 오라 오프셋 적용",
        }),
        new Rule("프롬프트에 (\"파란색\" + \"경고\" 또는 \"사이버펑크\" 또는 \"SF/IT\")가 포함된 경우:", new string[]
        {
            "→ 주요 잠재 목표: 데이터 흐름 텍스처 / 인터페이스 UI 패널 / 첨단 HUD 그래픽",
            "→ 색상 스펙트럼 범위: #0288D1 ~ #0D47A1",
            "→ 대기 환경 효과: 디지털 입자 노이즈 및 어두운 배경과의 명확한 대비 형성",
        }),
        new Rule("프롬프트에 (\"파란색\" + \"의상\" 또는 \"옷\")이 포함된 경우:", new string[]
        {
            "→ 주요 잠재 목표: 빈티지 데님 텍스처 / 제복·유니폼 직물 / 실크·광택 패브릭",
            "→ 색상 스펙트럼 범위: #1565C0 ~ #283593",
            "→ 표면 특성: 결에 따른 미세한 주름 음영과 명확한 섬유 질감 표현",
        }),
        new Rule("프롬프트에 (\"파란색\" + \"헤어\" 또는 \"머리모양\")이 포함된 경우:", new string[]
        {
            "→ 주요 잠재 목표: 애쉬 블루 / 딥 오션 블루 염색 머릿결의 결 방향성",
            "→ 색상 스펙트럼 범위: #42A5F5 ~ #1A237E",
            "→ 표면 광택도: 결을 따라 빛나는 은빛/플래티넘 블루(#E1F5FE) 하이라이트 자동 배치",
        }),
        new Rule("프롬프트에 (\"파란색\" + \"자연물\" 또는 \"바다\" 또는 \"얼음\" 또는 \"하늘\")가 포함된 경우:", new string[]
        {
            "→ 주요 잠재 목표: 수면의 파동 및 굴절 / 결정체 입자 / 투명한 얼음 질감",
            "→ 색상 스펙트럼 범위: #80DEEA ~ #00695C",
            "→ 표면 특성: 빛의 투과 및 굴절, 청량한 그라데이션 적용",
        }),
    };

    [MenuItem("Tools/Promes/Create Blue Guide Prefab")]
    public static void CreatePrefab()
    {
        // ------------------------------------------------------------------
        // 루트 오브젝트
        // ------------------------------------------------------------------
        GameObject root = new GameObject("Promes_Blue_Guide", typeof(RectTransform));
        ...
        // 제목 "Blue (파란색)"
        // ------------------------------------------------------------------
        CreateText(root.transform, "Title", "Blue (파란색)", 26, FontStyles.Bold, TitleColor, TextAlignmentOptions.TopLeft);
        ...
        string path = dir + "/Promes_Blue_Guide.prefab";
        ...
        if (target != null)
        {
            ...
            Debug.Log($"[PromesBlueGuidePrefabBuilder] '{target.name}' 하위에 Promes_Blue_Guide 인스턴스를 생성했습니다. 프리팹: {path}");
        }
        else
        {
            Debug.Log($"[PromesBlueGuidePrefabBuilder] 오브젝트를 선택하지 않아 씬 루트에 생성했습니다. Content 오브젝트 밑으로 옮겨주세요. 프리팹: {path}");
        }
